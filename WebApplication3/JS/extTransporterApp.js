...
.filter('currancyFormat', function () {
        return function (val) {

            var Amt = Number(String(val).replace(/\,/g, ""));
            var Removemi = Number(Amt).toLocaleString('hi-IN', {
                style: "currency",
                currency: "INR"
            });
            //var a = Removemi + ((Amt < 0) ? " Cr" : " Db");
            return String(Removemi).split(".")[0]; //String(a).replace("-", "");

            // return Math.abs(String(val).replace(/\,/g, ""));
        }
    })

// Contoller
.controller('transportersCtrl', function ($scope, $transporterQuote, SweetAlert, $timeout, $rootScope, toaster, $uibModal, $filter) {
        ...
        $transporterQuote.callAPI5('/api/transporter/verify/transporterURL',
            ...
            function (apiResponse) {
               ...
               // $scope.RFQList = apiResponse.data.RFQList;
                ...
                if ($scope.quoteObj && $scope.quoteObj.Components)
                    $scope.componentList = JSON.parse($scope.quoteObj.Components);
                $scope.oldComponentValuePairs = {};

                $scope.componentList.forEach(function (item) {
                        $scope.oldComponentValuePairs[item.componentID] = item.componentValue;
                });
                
                if ($scope.rfqObj.components)
                    $scope.rfqObj.components.forEach(function (item) {
                    ...
                    if (!$scope.quoteIssued) {
                        ...
                        item.ComponentValue = $scope.oldComponentValuePairs[item.ComponentID];
                        if (item.Currency == "INR" && item.Type == "Charge") 
                            $scope.componentTotal += Number(item.ComponentValue);
                        else if (item.Currency == "USD") {
                            if (item.Type == "Charge") 
                                $scope.componentTotal += Number(item.ComponentValue) * Number($scope.quoteObj.ExchangeRate);
                            $scope.exchangeRateShow = true;
                        }
                    }
                });
               ...
               // $scope.quotation.pickupDT = new Date($scope.rfqObj.pickUpDT).customFormat("#D#-#MMM#-#YY#");
                ...
                //console.log('$scope.quotation.ExtendDeliveryDT');
                //console.log($scope.quotation.ExtendDeliveryDT);

                if ($scope.externalParam[$scope.externalParam.length - 1] == 'canNotChangeQuote')
               ...
               // $scope.maxDeliveryDT = $scope.quotation.ExtendDeliveryDT;
                ...
                if ($scope.quoteObj)
                    $scope.isNegotiation = ($scope.quoteObj.NegotiationComments == "") ? false : true;
                else
                    $scope.isNegotiation = false;

                //console.log($scope.rfqObj);
                //console.log('$scope.quoteObj');
                //console.log($scope.quoteObj);
    ...
    $scope.setQuote = function () {
        ...
        $scope.rfqObj.components.forEach(function (item) {
            if (item.Type == "Charge") { 
                if (item.ComponentValue > $scope.oldComponentValuePairs[item.ComponentID]) {
                    toaster.info("You can reduce quote or keep it same. you can not increase the rate!");
                    isReturn = true;
                    return;
                }
                //else {
                //    $scope.componentTotal += Number(item.ComponentValue);
                //    $scope.componentTotal -= Number($scope.oldComponentValuePairs[item.ComponentID]);
                //}
            }
        });
        ...
        confirmSWALPopup(SweetAlert, "Are you sure you wish to submit this quote?", function () {
            var myParams = {
                ...
                negotiationComments: ($scope.quoteObj && $scope.quoteObj.NegotiationComments) ? $scope.quoteObj.NegotiationComments : '',
                charges: $scope.componentTotal
            }
    ...
    $scope.countComponentValue = function () {
        ...
        angular.forEach($scope.rfqObj.components, function (item, key) {
            if (item.ComponentValue && item.Currency == "INR" && item.Type == "Charge") {
                ...
                $scope.componentTotal += Number(item.ComponentValue) * Number($scope.quotation.exchangeRate);
            }
